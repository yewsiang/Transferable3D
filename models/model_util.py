...
g_onehotclass2type = {0: 'Car', 1: 'Pedestrian', 2: 'Cyclist'} # (YS)
...
SUN_NUM_HEADING_BIN = 12
SUN_NUM_SIZE_CLUSTER = 10 # one cluster for each type   
sun_type2class={'bed':0, 'table':1, 'sofa':2, 'chair':3, 'toilet':4, 'desk':5, 'dresser':6, 'night_stand':7, 'bookshelf':8, 'bathtub':9}
sun_class2type = {sun_type2class[t]:t for t in sun_type2class}
sun_type2onehotclass={'bed':0, 'table':1, 'sofa':2, 'chair':3, 'toilet':4, 'desk':5, 'dresser':6, 'night_stand':7, 'bookshelf':8, 'bathtub':9}
sun_type_mean_size = {'bathtub'      : np.array([0.765840,1.398258,0.472728]),
...
sun_mean_size_arr = np.zeros((SUN_NUM_SIZE_CLUSTER, 3)) # size clustrs
for i in range(SUN_NUM_SIZE_CLUSTER):
    sun_mean_size_arr[i,:] = sun_type_mean_size[sun_class2type[i]]
...
def get_box3d_corners_sunrgbd(center, heading_residuals, size_residuals):
    """ TF layer.
    Inputs:
        center: (B,3)
        heading_residuals: (B,NH)
    ...
    """
    batch_size = center.get_shape()[0].value
    heading_bin_centers = tf.constant(np.arange(0,2*np.pi,2*np.pi/SUN_NUM_HEADING_BIN), dtype=tf.float32) # (NH,)
    headings = heading_residuals + tf.expand_dims(heading_bin_centers, 0) # (B,NH)
    
    mean_sizes = tf.expand_dims(tf.constant(sun_mean_size_arr, dtype=tf.float32), 0) + size_residuals # (B,NS,1)
    sizes = mean_sizes + size_residuals # (B,NS,3)
    sizes = tf.tile(tf.expand_dims(sizes,1), [1,SUN_NUM_HEADING_BIN,1,1]) # (B,NH,NS,3)
    headings = tf.tile(tf.expand_dims(headings,-1), [1,1,SUN_NUM_SIZE_CLUSTER]) # (B,NH,NS)
    centers = tf.tile(tf.expand_dims(tf.expand_dims(center,1),1), [1,SUN_NUM_HEADING_BIN, SUN_NUM_SIZE_CLUSTER,1]) # (B,NH,NS,3)

    N = batch_size*SUN_NUM_HEADING_BIN*SUN_NUM_SIZE_CLUSTER
    corners_3d = get_box3d_corners_helper(tf.reshape(centers, [N,3]), tf.reshape(headings, [N]), tf.reshape(sizes, [N,3]))

    return tf.reshape(corners_3d, [batch_size, SUN_NUM_HEADING_BIN, SUN_NUM_SIZE_CLUSTER, 8, 3])
