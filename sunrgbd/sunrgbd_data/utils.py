...
def shift_box3d(box3d, shift_val):
    ''' Shifts a 3D box according to shift val
        Inputs:
            box3d: (8,3) array in upright depth coord.
            shift_val: (7,) center xyz, dims hwl, heading angle
    '''
    pass

def random_shift_box3d(box3d, shift_val=[0,0,0,0,0,0,0]):
    pass
...
def extract_pc_in_box3d(pc, box3d):
    ...
    def is_point_inside_bbox(pt, bbox):
        """
        Checks if point is inside a bounding box.
        """
        p1 = bbox[1,:] # Left corner
        p2 = bbox[2,:] # x direction of Left corner
        p4 = bbox[5,:] # y direction of Left corner
        p5 = bbox[0,:] # z direction of Left corner
        i = p2 - p1
        j = p4 - p1
        k = p5 - p1
        v = pt - p1
        return np.dot(v, i) > 0 and np.dot(v, i) < np.dot(i, i) and \
               np.dot(v, j) > 0 and np.dot(v, j) < np.dot(j, j) and \
               np.dot(v, k) > 0 and np.dot(v, k) < np.dot(k, k)

    def get_mask_of_points_inside_bbox(pts, bbox):
        pts_inside = []
        for pt in pts:
            if is_point_inside_bbox(pt, bbox):
                pts_inside.append(1)
            else:
                pts_inside.append(0)
        return np.array(pts_inside)

    try:
        box3d_roi_inds = in_hull(pc[:,0:3], box3d)
        pts_in_box = pc[box3d_roi_inds,:]
    except:
        print('======= Used alternative way to get pts in a box =======')
        mask = get_mask_of_points_inside_bbox(pc[:,0:3], box3d)
        box3d_roi_inds = (mask == 1)
        pts_in_box = pc[box3d_roi_inds,:]
    
    return pts_in_box, box3d_roi_inds

def compute_box_3d(obj, calib):
    ''' Takes an object and a projection matrix (P) and projects the 3d
        bounding box into the image plane.
        (YS) Produces box that is in upright_depth coordinates.
        Returns:
